...
def lab_request_page():
    ...
    if request.method == "POST":
        patient_uhid = request.form.get("patient_uhid", "").strip()

        print("ENTERED:", patient_uhid)
        print("ALL:", [p.uhid for p in Patient.query.all()])

        patient = Patient.query.filter_by(uhid=patient_uhid).first()

        if not patient:
            flash("Invalid Patient UHID.", 'error')
            return render_template(
                "lab_request.html",
        ...
        new_request = LabRequest(
            ...
            tests=json.dumps(request.form.getlist("tests")),
            status='Pending'
        )
        ...
        print("SAVED LAB REQUEST:", new_request.id)

        flash(f"Lab request created! Order ID: {new_request.id}", "success")
